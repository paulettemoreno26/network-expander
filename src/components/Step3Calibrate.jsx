...
const PHASE_META = {
  1: { label: 'Conversion', week: 'Week 1',    color: '#141417', bg: '#F2F2F2',
       desc: 'Rep visits accounts. Goal: get one Core SKU on shelf.' },
  2: { label: 'Retention',  week: 'Weeks 2–3', color: '#444444', bg: '#F2F2F2',
       desc: 'Validate placement and reinforce the relationship.' },
  3: { label: 'Active Drop', week: 'Week 4+',  color: '#141417', bg: '#E5FF01',
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addLayer({
        ...
        layout: { 'line-cap': 'round', 'line-join': 'round' },
        paint: { 'line-color': '#141417', 'line-width': 2.2, 'line-opacity': 0.65 },
      })
      ...
      map.addLayer({
        ...
        paint: {
          'circle-radius': 3, 'circle-color': '#999ea6', 'circle-opacity': 0.35,
      ...
      map.addLayer({
        ...
        paint: {
          ...
          'circle-color':   ['match', ['get', 'dotState'], 'active', '#141417', 'scoped', '#141417', '#999ea6'],
          'circle-opacity': ['match', ['get', 'dotState'], 'active', 1, 'scoped', 0.35, 0.15],
      ...
      map.addLayer({
        ...
        paint: {
          'circle-radius': 5.5, 'circle-color': '#141417',
      ...
      {options.map(opt => {
        ...
        return (
          <button key={opt.value} onClick={() => onChange(opt.value)} style={{
            ...
            border: `1.5px solid ${active ? '#141417' : 'var(--ka-color-border)'}`,
            transition: 'all 0.12s',
          }}>
            <span style={{ fontSize: 11, fontWeight: active ? 700 : 500, color: active ? '#141417' : 'var(--ka-color-text-tertiary)', whiteSpace: 'nowrap' }}>
              {opt.label}
            </span>
            <span style={{ fontSize: 9, color: active ? 'var(--ka-color-text-secondary)' : 'var(--ka-color-text-tertiary)', whiteSpace: 'nowrap' }}>
              {opt.desc}
            </span>
            {opt.recommended && (
              <span style={{
                position: 'absolute', top: -7, right: -5,
                fontSize: 7.5, fontWeight: 700, color: '#141417',
                background: '#E5FF01', border: '1px solid #141417',
...
export default function Step3Calibrate({
  ...
  return (
    <div style={{
      ...
      {/* ── Header KPI bar ── */}
      <div style={{
        flexShrink: 0, background: 'var(--ka-color-bg)', borderBottom: '1px solid var(--ka-color-border)', borderTop: '1px solid var(--ka-color-border)',
          ...
          {kpis.map((kpi, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ padding: '0 18px', paddingLeft: i === 0 ? 0 : 18 }} title={kpi.tip}>
                <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--ka-color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 3, whiteSpace: 'nowrap' }}>
                  {kpi.label}
                </div>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                  <span style={{ fontSize: 22, fontWeight: 900, color: kpi.color, letterSpacing: '-0.03em', lineHeight: 1 }}>
                    {kpi.value}
                  </span>
                  {kpi.unit && <span style={{ fontSize: 10, color: 'var(--ka-color-text-tertiary)' }}>{kpi.unit}</span>}
                </div>
              </div>
              {i < kpis.length - 1 && (
                <div style={{ width: 1, height: 32, background: 'var(--ka-color-border)', flexShrink: 0 }} />
              )}
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0, marginLeft: 16 }}>
          <button onClick={onBack} style={{
            background: 'transparent', color: 'var(--ka-color-text-secondary)', border: '1px solid var(--ka-color-border)',
          ...
          <button onClick={onNext} disabled={inReach.length === 0} style={{
            background: inReach.length > 0 ? 'var(--ka-color-primary)' : 'var(--ka-color-bg-layout)',
            color: inReach.length > 0 ? 'var(--ka-color-primary-text)' : 'var(--ka-color-text-disabled)',
      ...
      {/* ── Context strip ── */}
      <div style={{
        flexShrink: 0, background: 'var(--ka-color-bg)',
        padding: '6px 28px 8px', borderBottom: '1px solid var(--ka-color-border)',
      }}>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0, overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          display: 'flex', flexDirection: 'column',
          overflowY: 'auto', background: 'var(--ka-color-bg-layout)',
        }}>

          {/* Section 1 — Target Selection */}
          <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--ka-color-border)', background: 'var(--ka-color-bg-layout)' }}>
            <div style={{ fontSize: 9.5, fontWeight: 700, color: 'var(--ka-color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>
              Target Selection
            </div>

            {/* Segment */}
            <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--ka-color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 5 }}>
              Segment
            </div>
              ...
              {tierBreakdown.map(({ t, count, active }) => (
                <button key={t} onClick={() => toggleTier(t)} style={{
                  ...
                  <span style={{
                    ...
                    background: active ? '#141417' : 'var(--ka-color-border)',
                    transition: 'background 0.12s',
                  }} />
                  <span style={{ fontSize: 10.5, fontWeight: active ? 600 : 400, color: active ? '#141417' : 'var(--ka-color-text-tertiary)' }}>
                    {TIER_META[t].label}
                  </span>
                  <span style={{
                    ...
                    color: active ? 'var(--ka-color-text-secondary)' : 'var(--ka-color-text-tertiary)',
                  }}>{count}</span>
                </button>
              ))}
            </div>
              ...
              {routeBreakdown.map(({ r, count, active }) => {
                ...
                return (
                  <button key={r} onClick={() => toggleRoute(r)} style={{
                    ...
                    transition: 'all 0.12s',
                    background: active ? '#FFFFFF' : 'transparent',
                    border: `1px solid ${active ? 'var(--ka-color-border)' : 'var(--ka-color-border)'}`,
                    borderLeft: active ? '3px solid #141417' : '1px solid var(--ka-color-border)',
                    boxShadow: active ? '0 1px 4px rgba(0,0,0,0.06)' : 'none',
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                      <div style={{
                        width: 14, height: 14, borderRadius: '0.2rem',
                        background: active ? '#141417' : 'transparent',
                        border: `1.5px solid ${active ? '#141417' : 'var(--ka-color-border)'}`,
                      ...
                      <div>
                        <div style={{ fontSize: 11, fontWeight: active ? 600 : 400, color: active ? 'var(--ka-color-text)' : 'var(--ka-color-text-secondary)' }}>{r}</div>
                        {rep && <div style={{ fontSize: 9.5, color: 'var(--ka-color-text-tertiary)', marginTop: 1 }}>{rep}</div>}
                      </div>
                    </div>
                    <span style={{ fontSize: 10, color: 'var(--ka-color-text-tertiary)' }}>{count}</span>
                  </button>
                )
              })}
            </div>
          ...
          <div style={{ padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 16, background: 'var(--ka-color-bg-layout)' }}>
            <div style={{ fontSize: 9.5, fontWeight: 700, color: 'var(--ka-color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--ka-color-text)' }}>Route Deviation</span>
                <span style={{
                  fontSize: 10.5, fontWeight: 700, padding: '2px 8px', borderRadius: 6,
                  color: '#141417', background: '#F2F2F2', border: '1px solid #d4d5d6',
                }}>
              ...
              <div style={{ marginTop: 7, fontSize: 10.5, color: 'var(--ka-color-text-tertiary)', lineHeight: 1.5 }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--ka-color-text)' }}>Establishment Cycle</span>
                <span style={{
                  fontSize: 10.5, fontWeight: 700, padding: '2px 8px', borderRadius: 6,
                  color: '#141417', background: '#F2F2F2', border: '1px solid #d4d5d6',
                }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
          <div style={{ flex: 1, position: 'relative', minHeight: 0, background: '#E8E4DC', overflow: 'hidden' }}>
            <CalibrationMap strategy={strategy} filteredPool={filteredPool} inReach={inReach} routeDeviation={routeDeviation} />


            <div style={{
              position: 'absolute', top: 10, left: 12,
              background: 'rgba(255,255,255,0.92)', border: '1px solid var(--ka-color-border)',
              borderRadius: 'var(--ka-radius)', padding: '5px 10px',
              display: 'flex', flexDirection: 'column', gap: 4,
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#141417' }} />
                <span style={{ fontSize: 9.5, fontWeight: 600, color: 'var(--ka-color-text-secondary)' }}>In reach ({inReach.length})</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#141417', opacity: 0.35 }} />
                <span style={{ fontSize: 9.5, fontWeight: 600, color: 'var(--ka-color-text-tertiary)' }}>
                  Outside buffer ({filteredPool.length - inReach.length})
                </span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#999ea6', opacity: 0.5 }} />
                <span style={{ fontSize: 9.5, fontWeight: 600, color: 'var(--ka-color-text-tertiary)' }}>Filtered out</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#141417' }} />
                <span style={{ fontSize: 9.5, fontWeight: 600, color: 'var(--ka-color-text-secondary)' }}>Active POS</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{
          ...
          borderLeft: '1px solid var(--ka-color-border)',
          display: 'flex', flexDirection: 'column',
          overflow: 'hidden', background: 'var(--ka-color-bg-layout)',
        }}>
          {/* Tab bar */}
          <div style={{
            ...
            {[
              ...
              <button key={tab.id} onClick={() => setRightTab(tab.id)} style={{
                ...
                color: rightTab === tab.id ? 'var(--ka-color-text)' : 'var(--ka-color-text-tertiary)',
                borderBottom: `2px solid ${rightTab === tab.id ? 'var(--ka-color-text)' : 'transparent'}`,
          ...
          {/* Tab content */}
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 14, background: 'var(--ka-color-bg)' }}>
              ...
              <>
                ...
                {inReach.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    {[
                      ...
                      {
                        label: 'Success Estimate',
                        value: `${successProb}%`,
                        sub: `Based on ${persistence}-week cycle`,
                      },
                    ].map((row, i, arr) => (
                      <div key={i} style={{
                        display: 'flex', alignItems: 'flex-start', gap: 10,
                        padding: '10px 0',
                        borderBottom: i < arr.length - 1 ? '1px solid var(--ka-color-border)' : 'none',
                      }}>
                        <div style={{
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
                            <span style={{ fontSize: 10.5, fontWeight: 600, color: 'var(--ka-color-text-secondary)' }}>{row.label}</span>
                            <span style={{ fontSize: 12, fontWeight: 800, color: 'var(--ka-color-text)', whiteSpace: 'nowrap' }}>{row.value}</span>
                          </div>
                          {row.sub && <div style={{ fontSize: 10, color: 'var(--ka-color-text-tertiary)', marginTop: 2 }}>{row.sub}</div>}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {inReach.length > 0 && totalCAC > 0 && monthlyRevenue > 0 && totalCAC > monthlyRevenue && (
                  <div style={{
                    ...
                    borderLeft: '3px solid #141417', borderRadius: '0 0.5rem 0.5rem 0',
                    padding: '10px 12px',
                  }}>
                    <div style={{ fontSize: 9, fontWeight: 800, color: 'var(--ka-color-text)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>
                      ⚠ Value Alert
                    </div>
                    <p style={{ fontSize: 11, color: 'var(--ka-color-text-secondary)', lineHeight: 1.6, margin: 0 }}>
            ...
            {rightTab === 'forecast' && (
              <>
                ...
                <div style={{ background: 'var(--ka-color-bg-layout)', border: '1px solid var(--ka-color-border)', borderRadius: 'var(--ka-radius-lg)', padding: '14px' }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--ka-color-text)', marginBottom: 12 }}>
                    Projected Outcome
                  </div>
                  {[
                    ...
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                      <span style={{ fontSize: 11, color: 'var(--ka-color-text-secondary)' }}>{row.label}</span>
                      <span style={{ fontSize: 12, fontWeight: 800, color: 'var(--ka-color-text)' }}>{row.value}</span>
                    </div>
                  ))}
                </div>

                {/* Rule of 3 Tracker */}
                <div style={{ borderTop: '1px solid var(--ka-color-border)', paddingTop: 14 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                    <span style={{ fontSize: 9.5, fontWeight: 700, color: 'var(--ka-color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                      Rule of 3 Tracker
                    </span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                      <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#141417' }} />
                      <span style={{ fontSize: 9, fontWeight: 600, color: 'var(--ka-color-text-tertiary)' }}>Active</span>
                    </div>
                  </div>
                  ...
                  <div style={{ position: 'relative', display: 'flex', alignItems: 'flex-start' }}>
                    <div style={{
                      ...
                      background: 'var(--ka-color-border)', zIndex: 0,
                    }} />
                    {[1, 2, 3].map(n => {
                      ...
                      return (
                        <div key={n} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 1 }}>
                          <div style={{
                            ...
                            background: committed ? ph.bg : 'var(--ka-color-bg-layout)',
                            border: `2.5px solid ${committed ? ph.color : 'var(--ka-color-border)'}`,
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                            fontSize: 12, fontWeight: 900, color: committed ? ph.color : 'var(--ka-color-text-tertiary)',
                            boxShadow: 'none',
                            opacity: committed ? 1 : 0.45,
                          }}>{n}</div>
                          <div style={{ marginTop: 7, textAlign: 'center', opacity: committed ? 1 : 0.4 }}>
                            <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--ka-color-text)', marginBottom: 1 }}>{ph.label}</div>
                            <div style={{ fontSize: 9, color: 'var(--ka-color-text-tertiary)' }}>{ph.week}</div>
                            <div style={{ fontSize: 9, color: 'var(--ka-color-text-tertiary)', marginTop: 2, lineHeight: 1.4, maxWidth: 90 }}>{ph.desc}</div>
                          </div>
                        </div>
                      )
                    })}
                  ...
                  <div style={{
                    ...
                    ...(persistence >= 3
                      ? { background: '#E5FF01', color: '#141417', border: '1px solid #141417' }
                      : { background: 'var(--ka-color-bg-layout)', color: 'var(--ka-color-text-secondary)', border: '1px solid var(--ka-color-border)' }),
                  }}>
                    {persistence >= 3
                ...
                <div style={{ fontSize: 10.5, color: 'var(--ka-color-text-tertiary)', lineHeight: 1.6, fontStyle: 'italic' }}>
